...
- 📋 **brief-validator**
  ...
  除非一次委托带齐三层说明——要做什么、执行者怎么自查、评审者依据什么判断——否则会拦下这次对子 agent 的长篇委托。
...
- **默认全部本地运行** — 不发起任何网络请求，唯一的例外是 `recall` 的一个可选记忆层，只有你自己配置了 API key 才会启用。退避笔记存放在仅所有者可读的目录里。
