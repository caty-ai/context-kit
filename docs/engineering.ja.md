...
- **散文は英語のみ** — 是正メッセージの文章はすべて英語で書かれており、どのロケールでも同じように読めます。意図的な例外がひとつ: brief validator の既定セクショントークンは、上流の [family-dev-handbook](https://github.com/caty-ai/family-dev-handbook) 契約の日本語見出しまたは対応する英語のレイヤー名であり、`CK_BRIEF_REQUIRED_SECTIONS` で上書きできます。
